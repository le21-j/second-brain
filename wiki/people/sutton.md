---
title: Richard Sutton
type: person
tags: [rl, canon, alberta, deepmind]
course: [[python-ml-wireless]]
created: 2026-04-23
updated: 2026-04-23
---

# Richard Sutton

**Affiliation:** Professor, University of Alberta; Distinguished Research Scientist, DeepMind; Turing Award 2024 (with Andrew Barto, for "foundational work in reinforcement learning").
**Co-author of:** *Reinforcement Learning: An Introduction* (2nd ed., MIT Press, 2018) — with Andrew Barto. The RL canon.

## Why he matters to Jayden

**The RL canon is Sutton + Barto.** See [[sutton-barto-rl]]. The roadmap treats this book as the primary RL text (Phase 3 M7–8).

Sutton's big-picture essays are worth reading alongside the book:
- "The Bitter Lesson" (2019) — compute-scaling always wins over hand-designed priors. Influential short essay.
- "Reward is enough" (with Silver et al., 2021) — claim that all intelligence reduces to reward maximization. Controversial but foundational.

## Wireless connection

Ch 2 of the book (multi-armed bandits) is the conceptual ancestor of **regret matching** (Hart & Mas-Colell) — the algorithm in [[regretful-learning]] that powers Jayden's AirComp pipeline. If you can articulate "regret matching is to bandits what counterfactual utility is to observed reward," you understand both.

## Related
- [[sutton-barto-rl]]
- [[reinforcement-learning]]
- [[regretful-learning]]
- [[python-ml-wireless]]
