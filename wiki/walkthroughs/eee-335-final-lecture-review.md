...
> [!warning] **Don't memorize 20 formulas. Memorize 5 patterns.** Every single problem in Units 4–6 — single-stage gain, cascode, mirror-loaded diff amp, frequency response — is generated by the **same small set of building blocks**. McDonald writes problems that tweak the circuit slightly (add $R_s$, swap the load, take s.e. instead of differential, ask about $R_o$ instead of $A_v$) specifically to catch students who pattern-match a memorized formula instead of deriving from the framework. The students who survive draw the small-signal model and chug. **Five concepts beats twenty formulas.**

### The 5 patterns that generate every formula in Units 4–6

1. **Half-circuit analysis** — the keystone for diff amps and any symmetric circuit.
   - **Differential input** ($v_{G1} = +v_{id}/2, v_{G2} = -v_{id}/2$): the tail node is a **virtual ground** because $g_{m1} = g_{m2}$. Analyze one side as a CS amp with the source grounded.
   - **Common-mode input** ($v_{G1} = v_{G2} = V_{icm}$): the tail moves; each side sees source degeneration of $2 R_{SS}$. Analyze one side as a CS-with-$R_s$ amp using $R_s = 2 R_{SS}$.
   - This single trick turns *all* diff-amp problems into single-transistor problems you already know.

2. **Three building-block gains** — the primitives every other gain reduces to.
   - **Common-source (no degeneration):** $A_v = -g_m \,R_\text{out}$.
   - **Common-source with degeneration $R_s$:** $A_v = -R_\text{out} / (1/g_m + R_s) \approx -R_\text{out}/R_s$ when $g_m R_s \gg 1$.
   - **Diode-connected MOSFET impedance:** $1/g_m$ (looking into a gate-tied-to-drain node).
   - Plus the basic intrinsic small-signal numbers: $g_m = 2 I_D / V_{OV} = \sqrt{2 k_n' (W/L)\, I_D}$, $r_o = V_A / I_D$.

3. **How to find $R_\text{out}$ at any node** — just "look into" the node and combine impedances.
   - Looking into a drain (no source resistor): $r_o$.
   - Looking into a drain with source resistor $R_s$: $r_o(1 + g_m R_s) + R_s \approx g_m r_o R_s$ (cascode rule).
   - Looking into a source (gate grounded): $1/g_m$ (≈ low).
   - Single-ended diff-amp output (CM-loaded, looking up at $r_{o4}$ in parallel with looking down at $r_{o2}$): $r_{o2} \| r_{o4}$.
   - **Cascode total $R_o$:** $(g_{m2} r_{o2})\, r_{o1}$.

4. **The four definitions** — bookkeeping you must never confuse.
   - $A_d = v_o / v_{id}$ (differential gain — output divided by *differential* input).
   - $A_{cm} = v_o / v_{icm}$ (common-mode gain — output divided by *common-mode* input).
   - $\text{CMRR} = |A_d / A_{cm}|$ (figure of merit; in dB it's $20 \log_{10}$ of that ratio).
   - $A_d^\text{s.e.} = \tfrac{1}{2} A_d^\text{diff}$ (taking output at one drain instead of between the two halves the gain).

5. **The "what changes if…" reflex** — the exam-day lever.
   - "What if $R_D$ doubles?" → $A_v$ doubles, BW halves (if $R_o C_L$ pole dominates).
   - "What if the tail current source becomes ideal ($R_{SS} \to \infty$)?" → $A_{cm} \to 0$, CMRR $\to \infty$.
   - "What if we take differential output instead of single-ended?" → $A_d$ doubles, $A_{cm}$ goes from finite to zero (matched).
   - "What if we cascode the load?" → $R_o$ goes from $r_o$ to $g_m r_o^2$, gain goes from $g_m r_o$ to $(g_m r_o)^2$.
   - These all fall out of patterns 1–4. **Practice them.**

> [!tip] **What to memorize on the formula sheet vs. derive.** Memorize maybe **3–4 final results** that come up on every problem (so you don't burn time re-deriving them in 110 minutes):
> 1. $A_d = g_m (r_{o2} \| r_{o4})$ for the active-load diff pair.
> 2. $g_m = I_\text{tail}/V_{OV}$ for the diff pair.
> 3. $\text{CMRR}_\text{s.e.} = g_m R_{SS}$ (matched s.e. output).
> 4. $\omega_p = 1/(R_o C_L)$ (load pole in HF response).
>
> **Everything else** — CS with $R_s$, cascode $R_o$, diff-amp differential vs. s.e. mapping, mirror pole/zero ordering, common-mode range bounds — **derive from the framework above in 2–3 minutes during the exam.** The Master Cheat Sheet at the bottom of this page is a *map of where each formula lives*, not a target for rote memorization.

### How this maps to each unit

| Unit | Building blocks at play |
|---|---|
| **Unit 4** (CS / CG / SF) | Pattern 2 (gains) + Pattern 3 ($R_\text{out}$). Half-circuit doesn't appear yet. |
| **Unit 5** (cascode + freq response) | Pattern 3 (cascode $R_o$) + Pattern 5 (gain-bandwidth trade) + Miller / OCTC for HF. |
| **Unit 6** (diff pair + CM rejection + mirror load) | Pattern 1 (half-circuit) + Patterns 2–4 (gains, $R_\text{out}$, definitions). Pattern 5 dominates the qualitative MC questions. |

Read the rest of this document with the patterns in mind: each "Lecture N" section below tells you *which formulas live at that lecture*, but the **how-to-derive** lives in the five patterns above.
...
> [!warning] **Unit 6 is a framework-vs.-formulas trap.** Diff-amp problems have too many variants to memorize: MOS vs. BJT, resistor vs. current-mirror vs. current-source load, differential vs. single-ended output, with vs. without source degeneration, symmetric vs. mismatched, $A_d$ vs. $A_{cm}$ vs. CMRR. That's easily **20+ formulas** if you try to memorize every combination — and McDonald *will* tweak the circuit on the exam (add a degeneration resistor, swap the load, ask about $R_o$ instead of $A_v$) to catch students who pattern-match. **Don't.**
>
> Use the **5 patterns** from the top of this document:
> 1. **Half-circuit analysis** — diff input → tail virtual ground → CS half-circuit. CM input → tail moves → CS-with-$R_s$ half-circuit using $R_s = 2 R_{SS}$. **Single trick, every diff-amp problem.**
> 2. **Three gain primitives** — CS, CS-with-degeneration, diode-connected $1/g_m$. Every diff-amp gain reduces to one of these.
> 3. **$R_\text{out}$ at any node** — for the CM-loaded diff amp, the s.e. output sees $r_{o2} \| r_{o4}$. For a cascoded tail, $R_{SS} \approx g_{m5} r_{o5}^2$. Just look into the node.
> 4. **Definitions** — $A_d = v_o/v_{id}$, $A_{cm} = v_o/v_{icm}$, $\text{CMRR} = |A_d/A_{cm}|$. Mixing these up is the #1 way to lose points on Problem #4.
> 5. **"What changes if…"** — ideal tail ($R_{SS}\to\infty$) ⇒ $A_{cm}\to 0$ (diff out, matched). Differential output instead of s.e. ⇒ $A_d$ doubles, $A_{cm} \to 0$ (matched). Cascoded tail ⇒ $R_{SS}$ scales by $g_m r_o$, CMRR scales by $g_m r_o$.
>
> **Memorize 3–4 final results** ($A_d = g_m(r_{o2}\|r_{o4})$, $g_m = I/V_{OV}$, $\text{CMRR}_\text{s.e.} = g_m R_{SS}$, $\omega_{p1} = 1/(R_o C_L)$); **derive everything else** in 2–3 minutes from the framework. The pole-zero pair $(\omega_{p2}, \omega_z)$ in the mirror is the only HF result that genuinely benefits from rote memorization — see Lecture 36.
